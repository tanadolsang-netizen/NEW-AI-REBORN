    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[28px] border border-white/10 bg-white/[0.04] p-7 shadow-[0_1px_0_rgba(255,255,255,0.06)]">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setIsAuthOpen(false)} className="text-white/60 hover:text-white text-[12px]">Close</button>
        </div>
        <form onSubmit={submit} className="mt-5 grid gap-3">
          <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="email" type="email" required className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-[14px] text-white placeholder:text-white/40 outline-none focus:border-white/25 focus:ring-2 focus:ring-white/10 transition-all" />
          <input value={password} onChange={(e) => setPassword(e.target.value)} placeholder="password" type="password" required className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-[14px] text-white placeholder:text-white/40 outline-none focus:border-white/25 focus:ring-2 focus:ring-white/10 transition-all" />
